...
try:
    ...
    ser = serial.Serial('COM5', 9600, timeout=0.01)  # Replace 'COM_' with your COM serial port
    SERIAL_ENABLED = True
except serial.SerialException:
    SERIAL_ENABLED = False
    print("Attendance Button not found. Have Attendance Button ready to use and rerun program.\n\n")
...
# Attendance Status (0 - Ready, 1 - Busy)
status = 0  # Initializes as Ready
...
while True:
    ...
    if keyboard.is_pressed("a") or (SERIAL_ENABLED and ser.read() == b'a'):  # Attendance capture
        if SERIAL_ENABLED:
            # Attendance Status Busy
            status += 1
            ser.write(status)  # Sends Attendance Status to Arduino
